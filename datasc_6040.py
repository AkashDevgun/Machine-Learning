...
if __name__ == "__main__":
	...
	parameters = dict(
		xgboost__learning_rate=[0.02,0.04,0.06,0.1,0.15,0.2],
		xgboost__n_estimators=[700,1000,1200,1400,1600,1800,2000],
		xgboost__max_depth=[6,8,9,10],
		xgboost__subsample=[0.5,0.75,1],
		xgboost__gamma=[0,0.1,0.3]
		)

	cv = RandomizedSearchCV(pipeline, param_distributions=parameters, scoring='mean_squared_error', n_jobs=-1, verbose=10)

	cv.fit(X_dummytrain,y_dummytrain)

	predictions = cv.best_estimator_.predict(X_dummytest)
	print mean_squared_error(y_dummytest, predictions)
