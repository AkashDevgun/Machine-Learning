...
if __name__ == "__main__":
	...
	tfvec = TfidfVectorizer(stop_words='english', ngram_range=(1, 1), max_features = 2000)
	...
	t3 = pd.DataFrame(trainTFtagdata)
	t4 = pd.DataFrame(numericTraindfNew)

	d1 = pd.DataFrame(stringtest_X)
	d2 = pd.DataFrame(testtagdata)
	d3 = pd.DataFrame(testTFtagdata)
	d4 = pd.DataFrame(numericTestdfNew)
	...
	# t1 = np.hstack((stringtrain_X,traintagdata))

	# t2 = np.hstack((t1,trainTFtagdata))

	# FullTrain = np.hstack((t2,numericTraindfNew))

	# d1 = np.hstack((stringtest_X,testtagdata))

	# d2 = np.hstack((d1,testTFtagdata))

	# FullTest = np.hstack((d2,numericTestdfNew))
	...
	lsvc = ensemble.ExtraTreesRegressor(n_estimators = 300).fit(FullTrain, intvals)
	model = SelectFromModel(lsvc, prefit = True)

	Train_new = model.transform(FullTrain)
	print FullTrain.shape
	...
	FinalTrainLessFeature = FullTrain[np.ix_(np.arange(len(FullTrain)), newindices)]
	FinalTestLessFeature = FullTest[np.ix_(np.arange(len(FullTest)), newindices)]

	estimate = ensemble.GradientBoostingRegressor(alpha=0.1, n_estimators = 500, learning_rate = 0.1, max_depth = 10 , random_state = 0).fit(FinalTrainLessFeature, intvals)

	predictions = estimate.predict(FinalTestLessFeature)

	print "In writePredictions"

	dd = DictWriter(open("Predictions1.csv", 'w'),["target"])
	dd.writeheader()
	for y_val in predictions:
		dd.writerow({'target': y_val})		

	o = DictWriter(open("PredictionsEXGT.csv", 'w'),["id","quality"])
	o.writeheader()
	...
	X_dummytrain, X_dummytest, y_dummytrain, y_dummytest = train_test_split(FullTrain, intvals, test_size=0.2, random_state=42)
