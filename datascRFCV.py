...
if __name__ == "__main__":
	...
	spnumericTrain = sparse.csr_matrix(numericTraindfNew.values)
	spnumericTest = sparse.csr_matrix(numericTestdfNew.values)


	print spnumericTrain.shape
	print spnumericTest.shape
	...
	spstringTrain = sparse.csr_matrix(stringtrain_X)
	spstringTest = sparse.csr_matrix(stringtest_X)

	print spstringTrain.shape
	print spstringTest.shape
	...
	tfvec = CountVectorizer(stop_words='english', ngram_range=(1, 1), max_features = 25000)
	...
	traintagdata = vec.fit_transform(train['tags'].values)
	testtagdata = vec.transform(test['tags'].values)

	print traintagdata.shape
	print testtagdata.shape

	trainTFtagdata = tfvec.fit_transform(train['comments'].values)
	#trainTFtagdata = trainTFtagdataSp.toarray()
	print  trainTFtagdata.shape
	testTFtagdata = tfvec.transform(test['comments'].values)

	combined1 = hstack((spnumericTrain, spstringTrain))
	combined2 = hstack((combined1, traintagdata))
	FullTrain = hstack((combined2, trainTFtagdata))

	combined3 = hstack((spnumericTest, spstringTest))
	combined4 = hstack((combined3, testtagdata))
	FullTest = hstack((combined4, testTFtagdata))
	...
	# combined = hstack((traintagdata, trainTFtagdata))
	...
	# m1 = sparse.csr_matrix([[1, 2, 3], [4, 5, 6]])
	# x = m1.getrow(1).toarray()
	# print x
	# print x.shape
	# print A.shape
	# print m1.getrow(1)
	...
	print FullTrain.shape[0]
	print FullTest.shape[0]
	#print A.shape
	...
	# clflinear = linear_model.LinearRegression().fit(FullTrain,intvals)
	# predictions = clflinear.predict(FullTest)


	# print "In writePredictions"

	# o = DictWriter(open("PredictionsLinear.csv", 'w'),["id","quality"])
	# o.writeheader()
	# for ii, y_val in zip(intids, predictions):
	# 	o.writerow({'id': ii,'quality': y_val})

	# clfridge = Ridge(alpha=0.01).fit(FullTrain,intvals)
	# predictions = clfridge.predict(FullTest)


	# print "In writePredictions"


	# o = DictWriter(open("PredictionsRidge.csv", 'w'),["id","quality"])
	# o.writeheader()
	# for ii, y_val in zip(intids, predictions):
	# 	o.writerow({'id': ii,'quality': y_val})
	...
	# clfGBT = ensemble.GradientBoostingRegressor(alpha=0.1, n_estimators = 500, learning_rate = 0.1, max_depth = 10 , random_state = 0).fit(FullTrain,intvals)
	# predictions = []

	# for x in xrange(0,lengthTest):
	# 	predictions.append(clfGBT.predict(FullTest.getrow(x).toarray()))


	# print "In writePredictions"

	# o = DictWriter(open("PredictionsGBTalls.csv", 'w'),["id","quality"])
	# o.writeheader()
	# for ii, y_val in zip(intids, predictions):
	# 	o.writerow({'id': ii,'quality': y_val})
	...
	Ridge(alpha=0.01, max_iter = 20000),
	...
	for clf in featuredclassifierss:
		...
		for x in xrange(0,X_dummytest.shape[0]):
			df.loc[x] = estimate.predict(X_dummytest.getrow(x).toarray())
		...
		#print df
		#df['Predicted'] = df['Predicted'].str.lstrip('[').str.rstrip(']')
		print mean_squared_error(y_dummytest, df['Predicted'].values)
	...
	for clf in featuredclassifierss:
		...
		model = selector.fit(X_dummytrain,y_dummytrain)
		...
		FinalTrainLessFeature = X_dummytrain[:,newindices]
		FinalTestLessFeature = X_dummytest[:,newindices]
		# print X_dummytrain.getcol(1).toarray()
		...
		for cllf in featureclassifiers:
			...
			#print estimate.predict(FinalTestLessFeature)
			...
			for x in xrange(0,X_dummytest.shape[0]):
				df.loc[x] = estimate.predict(FinalTestLessFeature.getrow(x).toarray())
